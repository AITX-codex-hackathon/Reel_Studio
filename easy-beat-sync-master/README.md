...
Now run the program `EasyBeatSync.py` and pass the folder you created above:

```sh
python EasyBeatSync.py /path/to/project-folder
```

If you run the program without a folder path, it will try to open an optional GUI folder picker. Install `requirements-optional.txt` only if you need that no-argument picker.
